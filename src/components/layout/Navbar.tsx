...
const Navbar = () => {
  ...
  // Expose the real navbar height as a CSS variable so sticky in-page headers
  // (like the dashboard header/domain selector) can avoid overlapping it.
  useLayoutEffect(() => {
  ...
  return (
    ...
    <motion.header
      ...
      transition={{ duration: 0.6, ease: "easeOut" }}
       className={`fixed top-0 left-0 right-0 z-50 border border-transparent transition-[background-color,backdrop-filter,padding] duration-300 ${
        isScrolled
          ? "glass-card py-3"
          : "bg-transparent py-5"
      }`}
